...
import type { FestivalDiscoveryResponse, Recommendation, UserPreferences } from '@/types';
import type { IFestivalDiscoveryService, IRecommendationService } from './interfaces';
...
export class FestivalDiscoveryService implements IFestivalDiscoveryService {
     ...
     */
    async discoverArtists(festivalId: string, userPreferences: UserPreferences): Promise<FestivalDiscoveryResponse> {
        ...
        try {
            ...
            const recommendations = await this.recommendationService.generateRecommendations(festival, userPreferences);

            // Apply time preferences filter if specified
            const filteredRecommendations = this.applyTimePreferencesFilter(recommendations, userPreferences);
     ...
     * @returns Promise resolving to discovery response with recommendations
     */
    async discoverArtistsFromUrl(festivalUrl: string, userPreferences: UserPreferences): Promise<FestivalDiscoveryResponse> {
     ...
     */
    async refineRecommendations(sessionId: string, userPreferences: UserPreferences): Promise<Recommendation[]> {
     ...
     */
    private applyTimePreferencesFilter(recommendations: Recommendation[], userPreferences: UserPreferences): Recommendation[] {
