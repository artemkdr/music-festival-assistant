...
export class RecommendationService implements IRecommendationService {
  ...
  async generateRecommendations(
    ...
    for (const performance of performances) {
      ...
      if (score > 0.1) { // Only recommend artists with a meaningful score
    ...
    const sortedRecommendations = recommendations
      ...
      .slice(0, this.getMaxRecommendations(userPreferences.discoveryMode));
  ...
  async calculateArtistScore(
    ...
    const discoveryBonus = this.getDiscoveryModeBonus(
      userPreferences.discoveryMode, 
      artist.popularity
    );
  ...
  private genresMatch(genre1: string, genre2: string): boolean {
    ...
    for (const [parent, children] of Object.entries(genreRelations)) {
      if ((g1.includes(parent) && children.some(child => g2.includes(child))) ||
          (g2.includes(parent) && children.some(child => g1.includes(child)))) {
        return true;
      }
    }
  ...
  private async generateRecommendationReasons(
    ...
    // Genre matches
    const matchingGenres = artist.genre.filter(genre =>
      userPreferences.genres.some(userGenre => this.genresMatch(userGenre, genre))
    );
