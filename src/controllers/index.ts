...
import {
    festivalDiscoveryRequestSchema,
    userFeedbackSchema
} from '@/schemas';
...
export class FestivalDiscoveryController {
  ...
  async discoverArtists(request: NextRequest): Promise<NextResponse> {
    ...
    try {
      ...
      if (festivalId) {
        discoveryResponse = await this.festivalDiscoveryService.discoverArtists(
          festivalId,
          userPreferences as UserPreferences
        );
      } else if (festivalUrl) {
        discoveryResponse = await this.festivalDiscoveryService.discoverArtistsFromUrl(
          festivalUrl,
          userPreferences as UserPreferences
        );
      } else {
        return this.createErrorResponse(
          'Either festivalId or festivalUrl must be provided',
          [],
          400
        );
      }
  ...
  async refineRecommendations(request: NextRequest): Promise<NextResponse> {
    ...
    try {
      ...
      if (!sessionId || !userPreferences) {
        return this.createErrorResponse(
          'sessionId and userPreferences are required',
          [],
          400
        );
      }

      const refinedRecommendations = await this.festivalDiscoveryService.refineRecommendations(
        sessionId,
        userPreferences
      );
      ...
      return this.createSuccessResponse(
        'Recommendations refined successfully',
